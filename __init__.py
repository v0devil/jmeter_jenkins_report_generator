...
fromDirectory = "C:\\Users\\german.syomin\\Documents\\LiClipse Workspace\\jmeter\\resourses\\"
...
for df in dataframes:
    df.columns = ['elapsed', 'URL','responseCode','success','threadName','failureMessage','grpThreads','allThreads']
    ...
    agg[file_index] = byURL.aggregate({'elapsed':np.mean})
    agg[file_index]['count'] = byURL.success.count()
    if file_index != 0:
        agg[file_index]['count-diff'] = dataframes[0].groupby('URL').success.count()-dataframes[file_index].groupby('URL').success.count()
    agg[file_index]['average'] = byURL.elapsed.mean()
    if file_index != 0:
        agg[file_index]['average-diff'] = dataframes[0].groupby('URL').elapsed.mean()-dataframes[file_index].groupby('URL').elapsed.mean()
    agg[file_index]['median'] = byURL.elapsed.median()
    if file_index != 0:
        agg[file_index]['median-diff'] = dataframes[0].groupby('URL').elapsed.median()-dataframes[file_index].groupby('URL').elapsed.median()    
    agg[file_index]['75_percentile'] = byURL.elapsed.quantile(.75)
    agg[file_index]['90_percentile'] = byURL.elapsed.quantile(.90)
    agg[file_index]['99_percentile'] = byURL.elapsed.quantile(.99)
    agg[file_index]['maximum'] = byURL.elapsed.max()
    agg[file_index]['minimum'] = byURL.elapsed.min()
    agg[file_index]['%_errors'] = (1-df[(df.success == True)].groupby('URL')['success'].count()/byURL['success'].count())*100
    if file_index != 0:
        agg[file_index]['%_errors_diff'] = ((1-dataframes[0][(dataframes[0].success == True)].groupby('URL').success.count()/dataframes[0].groupby('URL').success.count())*100)-((1-dataframes[file_index][(dataframes[file_index].success == True)].groupby('URL').success.count()/dataframes[file_index].groupby('URL').success.count())*100)
...
for num in range(0,file_index):
    ...
    if num == 0:
        ...
        htmlfile.write("'>vs. "+jtl_files[num][2]+"</a></li>")
...
for num in range(0,file_index):
    ...
    # write header row. assumes first row in csv contains header
        if rownum == 0:
          ...
          #write all other rows    
            ...
            check_col = 0    
            for column in row:
                if (check_col==5 or check_col==7 or check_col==14) and num != 0:#diffs
                    try:
                    ...
                    if c_value<=0:
                        htmlfile.write('<td style="background-color:#66FF33">' + column + '</td>')
                    else:
    ...
    # write header row. assumes first row in csv contains header
        ...
        for server in server_names:
            ...
            #p95_rtot = df.groupby(pd.TimeGrouper(freq='10Min')).elapsed.quantile(.95)
    ...
    htmlfile.write("</tr>")
    htmlfile.write('<table>')    
    average_rtot = dataframes[num].groupby(pd.TimeGrouper(freq='10Min')).elapsed.mean()
    median_rtot = dataframes[num].groupby(pd.TimeGrouper(freq='10Min')).elapsed.median()
    ...
    font = {'family' : 'sans-serif',
      #  'weight' : 'bold',
        'size'   : 8}
    response_codes = dataframes[num].groupby("responseCode").elapsed.count()
    ...
    for URL in uniqueURL:
        ...
        average_rtot_url = dfURL.groupby(pd.TimeGrouper(freq='10Min')).elapsed.mean()
        median_rtot_url = dfURL.groupby(pd.TimeGrouper(freq='10Min')).elapsed.median()
